"""
【模块功能】
动态浏览器会话控制器，提供标准浏览器自动化功能。

【核心类】
- DynamicSession: 同步动态浏览器会话
- AsyncDynamicSession: 异步动态浏览器会话
...
- 资源拦截：支持禁用不必要的资源加载
- 域名屏蔽：支持屏蔽指定域名的请求
- 自定义自动化：支持通过page_action执行自定义自动化
...
"""
【模块功能】
动态浏览器会话控制器，提供标准Playwright浏览器自动化。
...
【主要特性】
- 页面池管理：支持多标签页并发管理
- 资源拦截：可选禁用字体、图片、样式表等资源
- 域名屏蔽：支持屏蔽指定域名及其子域名
- 代理轮换：支持自动代理轮换和重试
- 自动化支持：支持自定义页面操作回调
- CDP连接：支持通过CDP URL连接到现有浏览器

【使用场景】
用于需要完整浏览器渲染的场景，但不包含隐身功能。
对于需要绕过反爬虫检测的场景，请使用_stealth.py中的StealthySession。
"""
"""
【模块功能】
动态浏览器会话控制器，提供标准Playwright浏览器自动化。
